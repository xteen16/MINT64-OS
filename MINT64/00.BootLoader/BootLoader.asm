...
.SCREENCLEARLOOP:                       ; 화면을 지우는 루프
    ...
    push 0                              ; 화면의 Y 좌표(0)를 스택에 삽입
    push 0                              ; 화면의 X 좌표(0)를 스택에 삽입
    call PRINTMESSAGE                   ; PRINTMESSAGE 함수 call
    add  sp, 6                          ; 함수 종료 후 삽입한 파라미터 제거
...
READDATA:                               ; 디스크를 읽는 코드의 시작
    ; 모든 섹터를 다 읽었는지 확인
    cmp di, 0                           ; 복사할 이미지의 섹터 수를 0과 비교
    je  READEND                         ; 복사할 섹터 수가 0이라면 복사가 끝났으므로 READEND로 이동
...
READEND:
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    push LOADINGCOMPLETEMESSAGE         ; 출력할 메시지의 어드레스를 스택에 삽ㅇ비
    push 1                              ; 화면 Y 좌표(1)를 스택에 삽입
...
PRINTMESSAGE:
    ...
    push di                             ; 스택에서 삽입된 값을 꺼내 원래 값으로 복원
    push ax
    push cx
    push dx
...
.MESSAGEEND:
    ...
    pop di                              ; 자료구조(LIFO)이므로 삽입(push)의 역순으로
    pop si                              ; 제거 해야함     
    pop es
    pop bp
    ret
